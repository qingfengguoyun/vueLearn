...
<script lang='ts' setup>
    ...
    import type { Enemy, GameConfig } from "@/types/game";
        ...
        if (comData.value.isActive && !gameConfig.value.isGameover) {
            ...
            let id = setInterval(() => {
                ...
                if (comData.value.left + comData.value.width >= 0) {
                    comData.value.left -= comData.value.speed! * intervalTime / 1000
                    comData.value.hitbox_left -= comData.value.speed! * intervalTime / 1000
    ...
    function enemyInit() {
        ...
        comData.value.isActive = true;
        //修改障碍物速度
        comData.value.speed!+=(Math.random()*0.5)*comDataDefault.speed!
        console.log(comData.value.speed);
    }
