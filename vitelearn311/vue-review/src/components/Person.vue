...
import PersonClass from '@/utils/person.js';

let person = ref(new PersonClass("zhang", 18, [1, 2, 3]))
let person2 = ref(person.value)
...
watch(() => { return person.value.obj }, (newIns, oldIns) => {
    console.log("length", person.value.obj.length)
    if(isArrayHasDuplicates(newIns)){
        person.value.obj=arrayDuplicate(newIns)
    }
    // if(person.value.obj.length>=5){

    // }
}, { deep: true })

function isArrayHasDuplicates<T>(array: T[]): Boolean {
    let set = new Set(array);
    console.log(set.size,array.length)
    return set.size != array.length;
}

function arrayDuplicate<T>(array:T[]):Array<T>{
